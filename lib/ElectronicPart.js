 ...
 * Add participants for testing
  * @param {org.acme.electronicPartsNetwork.addParticipant} addParticipant - the
  * @transaction
  */
function addParticipant(addParticipant) {
  ...
  var ocms = [];
  var ocm1 = factory.newResource(NS, 'OriginalComponentManufacturer', 'ocm3@email.com');
  var ocm2 = factory.newResource(NS, 'OriginalComponentManufacturer', 'ocm4@email.com');

  ocm1.accountBalance = 100;
  ocm2.accountBalance = 200;
...
//to do:  create/add inventory asset
  ...
  let NS = 'org.acme.electronicPartsNetwork';
  let inventoryItem = factory.newResource(NS, 'ElectronicPartInventory', 'INV_001');
...
/*
  getAssetRegistry(NS + '.ElectronicPartInventory')
      .then(function (invRegistry) {
          // add the inventory
          return invRegistry.add(inventoryItem);
      });
*/
  return getAssetRegistry('org.acme.electronicPartsNetwork.Shipment')
  .then(function (shipmentRegistry) {
